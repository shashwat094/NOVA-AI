...
import { Send, User, Sparkles, MessageCircle, Mail, Phone, Heart, Coffee, Rocket, ExternalLink, MessageSquare, UserCircle2, FolderGit2, HandHeart, Copy, Check, Bug, Moon, Zap, Globe, GraduationCap, Dumbbell, ImagePlus, Wand2 } from "lucide-react";
...
const GREETING = "Nova's up and caffeinated. Throw a bug at me, ask what Shashwat's been building, or just say hi — I don't bite, I just occasionally misplace a semicolon. Oh, and hit the image icon if you want me to draw something.";
    ...
    if (mode === "image") {
      ...
      try {
        ...
        if (!response.ok || !data.image) {
          setMessages(m => [...m, { role: "assistant", content: `⚠️ ${data.error || "Image generation failed."}` }]);
        } else {
          setMessages(m => [...m, {
            role: "assistant",
            content: data.caption || "Here you go:",
    ...
    <div className="nova-app" style={{ "--mx": `${mouse.x}%`, "--my": `${mouse.y}%` }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .mode-toggle {
          width: 38px; height: 38px; border-radius: 10px; flex-shrink: 0; align-self: flex-end;
          background: #ffffff08; border: 1px solid var(--border); color: var(--text-dim);
          display: flex; align-items: center; justify-content: center; cursor: pointer;
          transition: all 0.15s;
        }
        .mode-toggle:hover { color: var(--text); border-color: #ffffff33; }
        .mode-toggle.active {
          background: linear-gradient(135deg, var(--ember), #f97316); color: white; border-color: transparent;
          box-shadow: 0 2px 10px #fb923c55;
        }
        @media (max-width: 640px) { .mode-toggle { width: 44px; height: 44px; } }

        .msg-image-wrap { margin-top: 8px; border-radius: 10px; overflow: hidden; border: 1px solid var(--border); }
        .msg-image { display: block; width: 100%; max-width: 320px; height: auto; }
          ...
          <div className="chat-shell tab-panel">
                  ...
                  <div className={`msg-bubble ${m.role}`}>
                    ...
                    {m.image && (
                      <div className="msg-image-wrap">
                        <img src={m.image} alt="Generated by Nova" className="msg-image" />
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="chat-input-row">
              ...
              <textarea
                ref={textareaRef}
                rows={1}
                placeholder={mode === "image" ? "Describe an image to generate..." : "Ask Nova anything — code, projects, whatever's on your mind"}
                value={input}
                onChange={e => { setInput(e.target.value); autoGrow(); }}
                onKeyDown={handleKeyDown}
              />
              <button className="chat-send" onClick={sendMessage} disabled={loading || !input.trim()}>
                {mode === "image" ? <Wand2 size={16} /> : <Send size={16} />}
              </button>
            </div>
          </div>
        )}
