...
export default function Nova() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, u => {
      const isUnverifiedPasswordAccount = u && !u.emailVerified && u.providerData.some(p => p.providerId === "password");
  ...
  async function handleEmailSignUp(e) {
    ...
    try {
      ...
      setGlobalBanner("Account created — check your inbox for a verification link, then log in.");
      setTimeout(() => setGlobalBanner(""), 8000);
    } catch (err) {
  ...
  async function handleEmailLogin(e) {
    ...
    try {
      ...
      if (!cred.user.emailVerified) {
        try { await sendEmailVerification(cred.user); } catch (_) { /* ignore resend failures */ }
        await signOut(auth);
        setAuthError("Please verify your email before logging in — we just sent another verification link to your inbox.");
        return;
      }
      setAuthModalOpen(false);
    ...
    <div className="nova-app">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .dropdown-backdrop { position: fixed; inset: 0; z-index: 39; background: #00000066; }
        .user-avatar-btn {
